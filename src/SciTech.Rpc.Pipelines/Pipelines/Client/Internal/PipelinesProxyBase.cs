...
{
    public class PipelinesProxyArgs : RpcProxyArgs
    {
        public PipelinesProxyArgs(
            PipelinesServerConnection connection,
...
#pragma warning disable CA1062 // Validate arguments of public methods
    public class PipelinesProxyBase : RpcProxyBase<PipelinesMethodDef>
    {
        ...
        private PipelinesServerConnection connection;

        private TaskCompletionSource<RpcPipelineClient>? connectTcs;

        protected PipelinesProxyBase(PipelinesProxyArgs proxyArgs, PipelinesMethodDef[] proxyMethods) : base(proxyArgs, proxyMethods)
        {
        ...
        public static PipelinesMethodDef CreateMethodDef<TRequest, TResponse>(
            RpcMethodType methodType,
            string serviceName, string methodName,
            IRpcSerializer? serializer,
            RpcClientFaultHandler? faultHandler)
        {
            return new PipelinesMethodDef(methodType, $"{serviceName}.{methodName}", typeof(TRequest), typeof(TResponse), serializer, faultHandler);
        }
        ...
        protected override ValueTask<IAsyncStreamingServerCall<TResponse>> CallStreamingMethodAsync<TRequest, TResponse>(
            TRequest request, PipelinesMethodDef method, CancellationToken ct)
        {
        ...
        protected override Task<TResponse> CallUnaryMethodImplAsync<TRequest, TResponse>(PipelinesMethodDef methodDef, TRequest request, CancellationToken cancellationToken)
        {
            IReadOnlyDictionary<string, string>? headers = this.CreateCallHeaders();
        ...
        protected override PipelinesMethodDef CreateDynamicMethodDef<TRequest, TResponse>(string serviceName, string operationName)
        {
        ...
        private IReadOnlyDictionary<string, string>? CreateCallHeaders()
        {
            ...
            if (nInterceptors > 0)
            {
                var metadata = new PipelinesCallMetadata();
        ...
        private class PipelinesCallMetadata : IRpcClientCallMetadata
        {
