    ...
    public class RpcCodeGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            if (context.SyntaxContextReceiver is RpcSyntaxReceiver rpcReceiver)
            {
                ...
                Dictionary<string, string> methodDefExpressions = new ();
                ...
                foreach (var typeSymbol in rpcReceiver.rpcInterfaces)
                {
                    ...
                    if (serviceInfo != null)
                    {
                        ...
                        var proxyBuilder = new RpcServiceProxyBuilder(allServices, context, methodDefExpressions);
...
");
                ...
                //{
                //    sb.AppendBlock(proxyType.MethodsArrayCode);
                //}
...
");
                ...
                foreach(var proxyType in proxyTypes )
                {
                    ...
                    sb.AppendBlock(proxyType.DictionaryEntryCode);
                    first = false;
                }
                if (!first) sb.AppendLine();
...
");
                sb.IncreaseIndent();
                first = true;
                foreach( var methodDefPair in methodDefExpressions)
                {
                    if (!first) sb.AppendLine(";");
                    sb.AppendLine($"internal static readonly SciTech.Rpc.Lightweight.Client.Internal.LightweightMethodDef {methodDefPair.Key} =");
                    sb.IncreaseIndent();
                    sb.AppendBlock(methodDefPair.Value);
                    sb.DecreaseIndent();
                    first = false;
                }
                if (!first) sb.AppendLine();
                ...
                context.AddSource("GeneratedProxies.Lightweight.g.cs", sb.ToString());

                foreach (var proxyType in proxyTypes)
                {
                    context.AddSource($"{proxyType.ProxyTypeName}.g.cs", proxyType.TypeCode);
                }
            }
