...
{
    public class TcpPipelinesEndPoint : IRpcPipelinesEndPoint
    {
        private RpcSocketServer? socketServer;

        private object syncRoot = new object();

        public TcpPipelinesEndPoint(string hostName, int port, bool bindToAllInterfaces)
        {
        ...
        {
            return new RpcServerConnectionInfo(this.DisplayName, new Uri( $"pipelines.tcp://{this.HostName}:{this.Port}" ), hostId);
        }
        ...
        public void Start(Func<IDuplexPipe, Task> clientConnectedCallback)
        {
            ...
            lock (this.syncRoot)
            {
                ...
                {
                    throw new InvalidOperationException("TcpPipelinesEndPoint already started.");
                }
